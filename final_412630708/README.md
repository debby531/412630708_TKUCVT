...
![ssh app 成功與 Docker 版本](ssh-and-versions.png)

![final-baseline snapshot](final-baseline-snapshot.png)
...
![兩次 build 對照，pip install 為 CACHED](build-cache-diff.png)
...
![階段 1：寫入後 SELECT](volume-3-stages-1.png)

![階段 2：down 後資料還在](volume-3-stages-2.png)

![階段 3：down -v 後資料消失](volume-3-stages-3.png)
...
![docker compose ps 與 healthz](hardening-verify-1.png)

![權限驗證 uid / CapEff / NoNewPrivs](hardening-verify-2.png)

![cgroup memory.max / cpu.max / pids.max](hardening-verify-3.png)
...
![F1 故障前](fault-A-before.png)

![F1 故障中](fault-A-during.png)

![F1 回復後](fault-A-after.png)
...
![F2 故障前](fault-B-before.png)

![F2 故障中](fault-B-during.png)

![F2 回復後](fault-B-after.png)
