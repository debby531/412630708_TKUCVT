...
**截圖：** `ssh-and-versions.png`、`final-baseline-snapshot.png`
...
**截圖：** `volume-3-stages-1.png`、`volume-3-stages-2.png`、`volume-3-stages-3.png`
...
- `memory.max: 268435456`、`cpu.max: 50000 100000`、`pids.max: 200`

**截圖：** `hardening-verify-1.png`、`hardening-verify-2.png`、`hardening-verify-3.png`
...
- **故障前：** 兩 service healthy，`/healthz` 回 ok（`fault-A-before.png`）
- **故障中：** db Exited，app Up (unhealthy)，`/healthz` 回 db unreachable（`fault-A-during.png`）
- **回復後：** `docker compose start db`，恢復 healthy（`fault-A-after.png`）
- **診斷推論：** db 停掉後 app 容器仍在跑，但 healthcheck 偵測 DB 連線失敗 → HTTP 503。unhealthy ≠ 容器死掉。

### 故障 2：F2 — stop app

- **注入方式：** `docker compose stop app`
- **故障前：** `curl /` 回 412630708（`fault-B-before.png`）
- **故障中：** app Exited，`curl` 回 Could not connect（connection refused）（`fault-B-during.png`）
- **回復後：** `docker compose start app`，恢復正常（`fault-B-after.png`）
